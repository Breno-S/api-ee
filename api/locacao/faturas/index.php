...
if ($method == 'GET') {
    ...
    // } else {
        ...
        $all_faturas = get_all_faturas($conn);

        if (!($all_faturas === false)) {
            $response['status'] = "200 OK";
            $response['message'] = "Todas as faturas do mês";

            if (count($all_faturas) == 0) {
                $response['data'] = [
                ...
                foreach ($all_faturas as $indice => $dados_fatura) {
                    array_push($response['data']['faturas'], $dados_fatura);
                }
            }
        } else {
            http_response_code(500);
            $response['status'] = "500 Internal Server";
            $response['message'] = "Erro ao obter todas as faturas do mês";
        }
    // }
} else {

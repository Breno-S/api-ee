...
if ($method == 'POST') {
	...
	if (isset($data['vaga']) && isset($data['placa'])) {

        // Validação dos dados
        if ( (filter_var($data['vaga'], FILTER_VALIDATE_INT) === false) ||
			 (is_string($data['placa']) === false) ) {
        ...
        if (procura_copia($conn, $data['placa'])) {
            http_response_code(409);
            $response['status'] = "409 Conflict";
            $response['message'] = "Carro já foi registrado";
            echo json_encode($response);
            exit;
        }

        if (registrar_entrada($conn, $data['vaga'], $data['placa'])) {
            $response['status'] = "200 OK";
            $response['message'] = "Entrada cadastrada";
        } else {
